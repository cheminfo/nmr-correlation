import { Correlation, CorrelationData, CorrelationOptions, Link } from '../types/types';
import generateID from './generateID';
  ...
  function hasAttachmentAtomType(correlation: Correlation, atomType: string): boolean {
    return correlation.attachment[atomType] && correlation.attachment[atomType].length > 0
      ? true
      : false;
  }

  function addAttachmentAtomType(correlation: Correlation, atomType: string): void {
    if (!hasAttachmentAtomType(correlation, atomType)) {
      correlation.attachment[atomType] = [];
    }
  }

  function setAttachments(correlation: Correlation, atomType: string, attachments: Array<number>): void {
    addAttachmentAtomType(correlation, atomType);
    correlation.attachment[atomType] = attachments;
  }

  function addAttachment(correlation: Correlation, atomType: string, attachment: number): void {
    addAttachmentAtomType(correlation, atomType);
  ...
  function removeAttachment(correlation: Correlation, atomType: string, attachment: number): void {
    if (hasAttachmentAtomType(correlation, atomType)) {
  ...
  function setCorrelation (data: CorrelationData, id: string, correlation: Correlation): CorrelationData {
  ...
  export {addAttachment, addAttachmentAtomType, addLink, buildCorrelation, hasAttachmentAtomType, hasLinks, removeAttachment, removeAttachments, removeLink, setAttachments, setCorrelation};
